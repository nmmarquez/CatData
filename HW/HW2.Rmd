...
Using the multinomial test for unordered data we can test whether the addition of new variables is warented using a likelihood ratio test for the multinomial model. Note that when we test the inclusion of the new variable we see a change in the degrees of fredom of 3. This is because we have three seperate functions that we are testing in the mutinomial distribution. Based on the likelihood ratio test their does not appear to be a signifigant difference in the outcome when we include SES so we will not include it in the final model. Our final model then predicts that John who has had seven life events has the following probability of each outcome.
...
We can see how the number of events changes the probabilities of outcomes in the plot below which spans a number of events equivelent to seen in the dataset.
...
If we fit a full model with all variables to a multinomial model, there is no inherent order to the type of prey, we find that only a few variables are significant in predicting what the prey may be. Below is a plot of the Log Odds($\beta$ coefficients) with confidence intervals  of the effect of observing a prey of a certain type in comparsion to the base category, which is birds. The two factors which seem to have an effect are size and being from Lake Hancock which decrease the probability of observing invertabrae prey in comparison to bird prey. Future analysis may consider running a likelihood ratio test to decide on the inclusion of this full variable set. The probability of a male alligator from Lake Trafford that is 3 meters long having a prey outcome of reptile is `r pReptile`.
